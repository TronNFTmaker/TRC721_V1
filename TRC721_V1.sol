...
/**
 * @title TRC-721 Non-Fungible Token enumeration extension logic
 */
 ...
 */
contract COLLECTIONNAME is TRC721, TRC721Enumerable, TRC721MetadataMintable {

    using Strings for uint256;

    constructor() public TRC721Metadata("Cubie", "CUBIE") {
      _setBaseURI(URI LINK);
     }
    ...
    function safePublicMint(address _to, uint256[] memory _tokenId) public payable {
      ...
      require(_tokenId.length < 5, "Only Max of 4 NFTS Tokens per person is allowed");
      require(_tokenId.length > 0, "No Cubie Tokens to mint");
      require(msg.value >= 3000000000 * _tokenId.length, "NFT price is 3000TRX per token");
